...
**Connection:** `ssh -i ~/.ssh/tc2_key tc2`
...
```bash
ssh -i ~/.ssh/tc2_key tc2 "squeue -u yutaek001"
```

### Detailed job queue (shows time limit, node, reason)
```bash
ssh -i ~/.ssh/tc2_key tc2 "squeue -u yutaek001 -la"
```

### Watch training output in real-time
```bash
# First, find the exact log filename
ssh -i ~/.ssh/tc2_key tc2 "ls -lt ~/3ddl/logs/ | head -10"

# Then tail it (replace with actual filename)
ssh -i ~/.ssh/tc2_key tc2 "tail -f ~/3ddl/logs/output_EXP-01_s42_train_12345.out"
```

### Check training progress from CSV log
```bash
# Last 5 lines of training log
ssh -i ~/.ssh/tc2_key tc2 "tail -5 ~/3ddl/experiments/EXP-01/seed42/train.log"

# Check L_sdf trend (epoch, L_sdf columns)
ssh -i ~/.ssh/tc2_key tc2 "awk -F',' 'NR==1 || NR%100==0' ~/3ddl/experiments/EXP-01/seed42/train.log"
```

### Check what GPU card your job is using
```bash
# Replace <jobid> with your SLURM job ID
ssh -i ~/.ssh/tc2_key tc2 "scontrol show -d jobid <jobid>" | grep JOB_GRES
```
...
```bash
ssh -i ~/.ssh/tc2_key tc2 "sacct -u yutaek001 --format=jobid,jobname%+20,state,elapsed,exitcode,reason%+20"
```

### Job history (all time, exclude batch steps)
```bash
ssh -i ~/.ssh/tc2_key tc2 "sacct -u yutaek001 --format=jobid,jobname%+20,qos,alloctres%+50,nodelist,start,elapsed,state,reason%+20 | grep -wv batch"
```

### Job efficiency report (CPU/memory/GPU utilization)
```bash
ssh -i ~/.ssh/tc2_key tc2 "seff <jobid>"
```

### Custom job history script (if available on TC2)
```bash
ssh -i ~/.ssh/tc2_key tc2 "myjobhistory"
```
...
```bash
ssh -i ~/.ssh/tc2_key tc2 "ls -la ~/3ddl/experiments/EXP-01/seed42/results.json"
```

### View results summary
```bash
ssh -i ~/.ssh/tc2_key tc2 "python3 -m json.tool ~/3ddl/experiments/EXP-01/seed42/results.json | head -30"
```

### Download results to local machine
```bash
# Single experiment
scp -i ~/.ssh/tc2_key tc2:~/3ddl/experiments/EXP-01/seed42/results.json experiments/EXP-01/seed42/

# All results at once
scp -i ~/.ssh/tc2_key -r tc2:~/3ddl/experiments/ experiments/

# Training log only
scp -i ~/.ssh/tc2_key tc2:~/3ddl/experiments/EXP-01/seed42/train.log experiments/EXP-01/seed42/
```

### Download reconstructed meshes
```bash
scp -i ~/.ssh/tc2_key -r tc2:~/3ddl/experiments/EXP-01/seed42/reconstructions/ experiments/EXP-01/seed42/reconstructions/
```
...
```bash
# Find error files
ssh -i ~/.ssh/tc2_key tc2 "ls -lt ~/3ddl/logs/error_* | head -10"

# View specific error log (replace with actual filename)
ssh -i ~/.ssh/tc2_key tc2 "cat ~/3ddl/logs/error_EXP-01_s42_train_12345.err"
```
...
### Check disk usage
```bash
ssh -i ~/.ssh/tc2_key tc2 "du -sh ~/3ddl/"
ssh -i ~/.ssh/tc2_key tc2 "du -sh ~/3ddl/experiments/ ~/3ddl/data/ ~/.conda/"
```
...
```bash
ssh -i ~/.ssh/tc2_key tc2 "find ~/3ddl/experiments/ -name 'epoch_*.pt' -delete"
```

### Check total quota usage
```bash
ssh -i ~/.ssh/tc2_key tc2 "df -h /home/msai/yutaek001 2>/dev/null || du -sh ~/"
```
...
```bash
ssh -i ~/.ssh/tc2_key tc2 "scancel <jobid>"
```

### Cancel all your jobs
```bash
ssh -i ~/.ssh/tc2_key tc2 "scancel -u yutaek001"
```

### Cancel only pending jobs (keep running ones)
```bash
ssh -i ~/.ssh/tc2_key tc2 "scancel -u yutaek001 -t PENDING"
```
...
```bash
ssh -i ~/.ssh/tc2_key tc2 "sinfo -N -l"
```

### Check specific nodes in MGPU-TC2 partition
```bash
ssh -i ~/.ssh/tc2_key tc2 "scontrol show node TC2N0[1-6]" | grep -E "NodeName|State|AllocTRES|FreeMem"
```

### Find idle nodes
```bash
ssh -i ~/.ssh/tc2_key tc2 "sinfo -N -p MGPU-TC2 | grep idle"
```
...
```bash
ssh -i ~/.ssh/tc2_key tc2 "du -sh ~/.conda/envs/yt3dl/"
```
...
rsync -avz -e "ssh -i ~/.ssh/tc2_key" --exclude='data/' --exclude='.git/' --exclude='experiments/' \
    /Users/lcpnine/3ddl/ tc2:~/3ddl/
```

### Upload specific files
```bash
scp -i ~/.ssh/tc2_key src/train.py tc2:~/3ddl/src/
scp -i ~/.ssh/tc2_key configs/config.yaml tc2:~/3ddl/configs/
```

### Download everything
```bash
rsync -avz -e "ssh -i ~/.ssh/tc2_key" tc2:~/3ddl/experiments/ experiments/
```
...
# 2. Submit (on TC2)
ssh -i ~/.ssh/tc2_key tc2 "cd ~/3ddl && ./slurm/submit.sh EXP-01 42"
...
```bash
ssh -i ~/.ssh/tc2_key tc2 "echo '=== JOBS ===' && squeue -u yutaek001 2>/dev/null; echo '=== DISK ===' && du -sh ~/3ddl/experiments/ ~/3ddl/data/ 2>/dev/null; echo '=== LATEST RESULTS ===' && ls -lt ~/3ddl/experiments/*/seed*/results.json 2>/dev/null | head -5"
```
