...
\begin{itemize}
    \item \textbf{Eikonal regularization improves CD-based label efficiency}: relative to the non-Eikonal baseline (EXP-01, CD = 0.0361, NC = 0.729), Eikonal improves full-label CD (EXP-02, CD = 0.0295) while slightly reducing NC, gives the strongest overall no-PE result at 50\% supervision (EXP-03, CD = 0.0288, NC = 0.748), and keeps 10\% and 5\% supervision numerically competitive. Across matched comparisons, the NC effect is neutral to slightly negative except for the 50\% point.
    \item \textbf{The 10\% no-PE regime is mixed rather than definitive}: EXP-04 (10\% supervision, 3 seeds) achieves CD = 0.0315 $\pm$ 0.0015 and NC = 0.7288 $\pm$ 0.0032, but qualitative inspection is less convincing than the metrics alone suggest, so we do not treat 10\% supervision as the strongest visual operating point.
...
In these equations, $N_s$ and $N_u$ denote the supervised and unsupervised counts in the current minibatch, not the full preprocessed point pools.
...
(0.0288 / 0.748). In the representative mesh comparison, this configuration remains visually competitive, although the examples should be read as illustration rather than proof of uniform qualitative superiority.
Because EXP-03 was trained for 1{,}500 epochs while EXP-02 used 3{,}000 epochs,
...
visually reliable under the post-fix budget.
...
stability, but the per-shape comparisons reveal mixed visual quality. For the
final written claim, we therefore treat 50\% + Eikonal as the clearest supported
metric result, and 10\% + Eikonal as promising but visually less reliable.

Qualitative inspection (Fig.~\ref{fig:mesh_comparison}) changes the emphasis of the story. The 50\% Eikonal run is competitive in the shown examples, but it is not uniformly better than every lower-label or non-Eikonal reconstruction on every shape. This is exactly why the final conclusion is more conservative than a metric-only reading of EXP-04.
...
The most plausible explanation is that Fourier PE interacts badly with the training/evaluation geometry of this setup. Post-fix marching cubes is clipped to the unit sphere, so the remaining failure is not the pre-fix cube-corner extrapolation problem. Instead, PE appears to induce oscillatory gradients within the evaluated unit-sphere region: the zero level set can remain close enough for CD to survive, while the surface-normal direction becomes unreliable enough for NC to degrade.
...
result is 50\% supervision with Eikonal, which gives the best no-PE metrics
and remains visually competitive in representative reconstructions. Third, 10\% supervision with Eikonal
