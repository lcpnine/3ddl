...
\begin{abstract}
Neural implicit representations such as DeepSDF learn continuous signed distance functions for 3D shape reconstruction, but require dense signed distance supervision for every training point. We investigate whether Eikonal regularization can compensate for reduced supervision, and whether Fourier positional encoding (PE) helps or hurts in this regime. Using a corrected preprocessing pipeline, we train on 299 ShapeNet shapes (airplane, chair, table) at supervision ratios from 5\% to 100\% and evaluate with Chamfer Distance (CD) and Normal Consistency (NC) on training-set reconstructions. The main result is that Eikonal regularization improves CD-based label efficiency, with the strongest reduced-label operating point at 50\% supervision (EXP-03: CD = 0.0288, NC = 0.748), while a 3-seed 10\% run remains numerically competitive but visually less reliable. In contrast, Fourier PE primarily collapses normal consistency at the 10\% supervision slice we were able to retrain, while leaving CD broadly comparable to the no-PE baseline; lowering the encoding frequency from $L=6$ to $L=4$ partially recovers NC at a clear CD cost. We therefore conclude that Eikonal regularization is the useful ingredient for reducing signed-distance labels in this DeepSDF setup, whereas Fourier PE is not recommended at 10\% supervision.
\end{abstract}
...
We report only the corrected post-fix experiments on 299 ShapeNet shapes (airplane, chair, table); earlier runs used a thin near-surface-only sampling band and are excluded throughout. Our key findings are:
\begin{itemize}
    \item \textbf{Eikonal regularization improves CD-based label efficiency}: relative to the non-Eikonal baseline (EXP-01, CD = 0.0361, NC = 0.729), Eikonal improves full-label CD (EXP-02, CD = 0.0295) while slightly reducing NC, and keeps 50\%, 10\% and 5\% supervision numerically competitive.
    \item \textbf{The 10\% no-PE regime is mixed rather than definitive}: the 3-seed run is numerically competitive but qualitative inspection is less convincing than the metrics alone suggest, so we do not treat 10\% supervision as the strongest visual operating point.
    \item \textbf{Fourier PE degrades NC at 10\% supervision} while leaving CD close to the no-PE baseline; lowering $L$ from 6 to 4 partially recovers NC at a clear CD cost.
\end{itemize}
...
\textbf{SDF loss} ($\mathcal{L}_{\text{sdf}}$): L1 regression on supervised minibatch points only:
\begin{equation}
...
\textbf{Second-order loss} ($\mathcal{L}_{\text{2nd}}$): Optionally penalizes the divergence of the gradient field on the same minibatch points as $\mathcal{L}_{\text{eik}}$ to encourage smoother surfaces:
\begin{equation}
    \mathcal{L}_{\text{2nd}} = \frac{1}{N_s + N_u} \sum_k |\nabla \cdot \nabla_x f_\theta(z_i, x_k)|
\end{equation}
This term is implemented in the codebase but not exercised in the corrected
post-fix experiment matrix; the planned EXP-08 ablation was not retrained.
...
Training iterates over all shapes per epoch, sampling a batch of 16{,}384 points per shape split evenly between supervised and unsupervised samples. We use the Adam optimizer with learning rate $5 \times 10^{-4}$, decayed by $0.5\times$ every 500 epochs, and gradient clipping at \texttt{max\_norm}~=~1.0.
...
All experiments were trained on a single NVIDIA A40 GPU in the NTU TC2 cluster. Hyperparameters are shared across runs as specified in Section~\ref{sec:method}.

%% ============================================================
\section{Results}
%% ============================================================

All numbers in this section are from the corrected pipeline (Section~\ref{sec:dataset}) and evaluated on 299 training shapes.

\subsection{Eikonal Improves Label Efficiency, but 50\% Is the Clearest Operating Point}
...
improves CD to 0.0295 but slightly lowers NC to 0.721. The strongest no-PE operating point is EXP-03 at 50\%
supervision, which attains the best CD/NC pair among the non-PE runs
(0.0288 / 0.748). Because EXP-03 was trained for 1{,}500 epochs while EXP-02 used 3{,}000 epochs,
the small quantitative gap between the 50\% and 100\% Eikonal runs should not
be interpreted as a controlled proof that 50\% supervision is intrinsically
better; the stronger claim is that 50\% supervision remains competitive and
visually reliable.
...
04 & 10\% + Eik (3-seed) & 0.0315{\scriptsize$\pm$0.0015} & 0.7288{\scriptsize$\pm$0.0032} \\
...
EXP-04 (3 seeds) remains numerically competitive with EXP-01 and is seed-stable (CV(CD)~=~0.047), but qualitative inspection (Fig.~\ref{fig:mesh_comparison}) reveals mixed visual quality, with the 50\% Eikonal run not uniformly better than every lower-label or non-Eikonal reconstruction on every shape. We therefore treat 50\% + Eikonal as the clearest supported metric result, and 10\% + Eikonal as promising but visually less reliable.

\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/mesh_comparison.png}
    \caption{Representative reconstructions by experiment. The no-PE Eikonal
    runs remain visually competitive with the baseline, but the figure should
    not be read as proving a uniform qualitative ordering across all shapes. The
    PE columns show the clearest qualitative degradation: thin structure
    disappears, surfaces become unstable, and reducing the PE frequency does
    not repair the failure mode.}
...
Within the 10\% slice (Table~\ref{tab:results}), the dominant effect of PE is on normal consistency rather than chamfer distance: EXP-06 ($L=6$) matches the no-PE EXP-04 on CD but its NC drops from 0.729 to 0.517, reproducible across all three seeds (CV(NC) $\approx$ 0.013). EXP-11 ($L=4$) partially recovers NC to 0.609 at the cost of CD rising to 0.0432, moving the operating point toward the no-PE baseline on NC but away from it on CD rather than producing a uniformly better PE configuration. We cannot state from corrected data that PE collapses CD across supervision ratios; we can state that, at 10\% supervision, PE primarily degrades the gradient field of the learned SDF (NC) rather than the surface localization itself (CD).

\subsection{Per-Category Breakdown}
\label{sec:per_category}

Fig.~\ref{fig:per_category} breaks down the post-fix no-PE subset and the 10\% PE runs into per-shape CD and NC distributions by ShapeNet category. Three observations reinforce the aggregate numbers. (i)~Category difficulty is consistent in one direction: airplanes are usually the easiest on CD, while chairs and tables alternate as the hardest category depending on the run. (ii)~Eikonal regularization compresses the category spread on both CD and NC (EXP-02 and EXP-03), i.e.\ its benefit is broadly distributed rather than concentrated in one category. (iii)~The NC drop under Fourier PE (EXP-06, EXP-11) is broadly distributed across categories rather than driven by one category, consistent with PE acting on the decoder's gradient field rather than on category-specific geometry.
...
improves CD-based label efficiency relative to the non-Eikonal baseline. EXP-02 shows
that Eikonal already improves CD at full supervision, although NC is slightly
lower, and EXP-03 shows that the model can retain strong quantitative results
even after halving the labels. The 10\% and 5\% runs remain numerically competitive, but the figures
...
Two secondary observations constrain this interpretation. First, the NC evidence
is not a matched improvement story: EXP-02 has lower NC than EXP-01, and EXP-04
matches EXP-01's NC to three decimals despite using only 10\% of the labels.
Second, the ratio-dependent warmup schedule appears necessary in practice,
although we did not run a clean ablation on warmup itself.

\subsection{Why Fourier PE Costs Normal Consistency}
\label{sec:why_pe}

The post-fix evidence at 10\% supervision is that Fourier PE primarily degrades the gradient field of the learned SDF rather than surface localization. The most plausible explanation is that PE interacts badly with the training/evaluation geometry of this setup: marching cubes is clipped to the unit sphere, so the remaining failure is not cube-corner extrapolation. Instead, PE appears to induce oscillatory gradients within the evaluated unit-sphere region: the zero level set can remain close enough for CD to survive, while the surface-normal direction becomes unreliable enough for NC to degrade. Lowering the frequency from $L=6$ to $L=4$ partially mitigates the NC drop at the cost of higher CD; we cannot generalize this trade-off to 5\% or 100\% supervision from the post-fix data.
...
regularization is useful: it improves the full-label baseline on CD and enables strong
performance at reduced supervision. Second, the most convincing label-efficiency
result is 50\% supervision with Eikonal, which gives the best no-PE metrics; 10\% supervision is numerically competitive but qualitatively less reliable. Third, Fourier PE is not recommended at the 10\% slice we could test, with both $L=6$ and $L=4$ degrading NC relative to the no-PE Eikonal baseline.

The most natural next steps are therefore (i)~closing the PE matrix at 100\% and 5\% supervision and (ii)~exploring alternative high-frequency representations such as multi-resolution hash encodings~\cite{instantngp} or revised sampling strategies that reduce the observed mismatch between training support and reconstruction queries.
