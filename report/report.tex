...
\begin{itemize}
    ...
    \item \textbf{Fourier PE degrades NC at 10\% supervision}: post-fix EXP-06 ($L=6$, 3 seeds) achieves CD = 0.0308 $\pm$ 0.0012 and NC = 0.5171 $\pm$ 0.0066, and post-fix EXP-11 ($L=4$, single seed) achieves CD = 0.0432 and NC = 0.6090. CD remains close to the no-PE 10\% baseline; NC drops noticeably; lowering the PE frequency from $L=6$ to $L=4$ partially recovers NC at a small CD cost. PE at 5\% and 100\% supervision was not retrained under the post-fix QoS budget, so the PE conclusion in this paper is restricted to 10\% supervision.
\end{itemize}
...
\begin{figure}[t]
    ...
    \caption{Subset ablation plot retained for context; the authoritative post-fix PE numbers used in the conclusion are in Table~\ref{tab:results}.}
    \label{fig:ablation}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{10\% PE-frequency subset plot retained for context only.}
    \label{fig:pe_freq}
\end{figure}
...
\midrule
EXP-04 (10\% + Eik) & 0.0315 & 0.0015 & 0.047 & 0.7288 \\
EXP-06 (10\% + Eik + PE $L=6$) & 0.0308 & 0.0012 & 0.040 & 0.5171 \\
\bottomrule
\end{tabular}
\end{table}
