...
SHAPES = ["airplane_0009", "chair_0069", "table_0047"]
SEED = "seed42"
...
def main() -> None:
    ...
    for r, shape in enumerate(SHAPES):
        ...
        render_mesh(axes[r][0], load_mesh(gt_path) if gt_path else None,
                    "GT" if r == 0 else "", azim=args.azim)
        for c, (exp, label) in enumerate(EXPERIMENTS, start=1):
            recon_path = EXP_DIR / exp / SEED / RECON_SUBDIR / f"{shape}.obj"
            mesh = load_mesh(recon_path)
            render_mesh(axes[r][c], mesh, label if r == 0 else "", azim=args.azim)
